...
for appname in appnames_set:
    for spinlooppause_val in spinloop_set:
        dfs = []
        for file in files:
            if appname in file and spinlooppause_val in file:
                df = pd.read_csv(file)
        ...
        for connection_count in set([c for c in df['connections'].unique()]):
            ...
            for i, df in enumerate(dfs):
                ...
                # plt.savefig('{}_{}.png'.format(appname, spinlooppause_val))
                    
            ax.set_xlabel('Connections + Concurrency')
            ax.set_ylabel('Requests per second')
            ax.set_title('{} - {} requests - spinlooppause={}'.format(appname, connection_count, spinlooppause_val))
            ax.legend(loc='upper left')
            plt.tight_layout()
            plt.savefig('{}_{}_{}.png'.format(appname, spinlooppause_val, connection_count))
