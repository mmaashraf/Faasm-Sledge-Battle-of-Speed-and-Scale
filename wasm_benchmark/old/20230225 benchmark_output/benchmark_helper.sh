...
function start_faasm () {
	host=$1
	core_count=$2
	echo -n "Starting Faasm with $core_count worker(s) on host $1"
...
function start_track_cpu_mem () {
	...
	source /mydata/monitor.sh $framework $datafile 1>/mydata/cpumem.log 2>/mydata/cpumem.log &
	echo \$!
	EOF
}
...
function generate_ab_graph_all_config () {
	...
	# mv "${app_name}"*".png" "benchmark_output/faasm_${app_name}"
