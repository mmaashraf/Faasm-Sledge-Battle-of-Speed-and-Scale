...
#         virt_memory=$(sudo pmap -x $pid | tail -n 1 | awk '{print $3/1024}') # virtual mb
#         phys_memory=$(sudo pmap -x $pid | tail -n 1 | awk '{print $4/1024}') # rss mb
#         cpu_sum=$(echo "$cpu_sum+$cpu" | bc -l)
#         vmem_sum=$(echo "$vmem_sum+$virt_memory" | bc -l)
#         pmem_sum=$(echo "$pmem_sum+$phys_memory" | bc -l)
#     done
#     # if cpu/mem are valid numbers, save to file
#     if [[ "$cpu_sum" =~ ^[0-9]+(\.[0-9]+)?$ ]]; then
#         echo "$i $cpu_sum $vmem_sum $pmem_sum" >> ${data_file}
#     fi
#     i=$((i+1))
# done
